...
class RequestController extends Controller
{
     ...
     * - Tidak ada OCR data dari session
     * - Tidak ada preview upload
     * - Form selalu dimulai dari kosong
     * - Upload foto surat bersifat OPSIONAL di dalam form
     */
     ...
     * 
     * PERUBAHAN: OCR DINONAKTIFKAN
     * - Tidak ada temporary file dari OCR scan
     * - Upload foto surat OPSIONAL langsung dari form
     */
    public function storeSikmb(SubmitSikmRequest $request)
    {
        try {
            Log::info('VENDOR_STORE_SIKMB_START', [
                'user_id' => Auth::id(),
                'request_data' => $request->except(['original_form_image']),
            ]);
            ...
            // NOTE: Upload foto surat OPSIONAL dari form field
            // Validasi sudah ada di SubmitSikmRequest (nullable)

            Log::info('VENDOR_STORE_SIKMB_BEFORE_SERVICE', [
                'user_id' => Auth::id(),
                'vendor_id' => $vendor->id,
                'data_keys' => array_keys($data),
                'has_image' => isset($data['original_form_image']),
            ]);
    ...
    public function storeSik(SubmitSikRequest $request)
    {
        try {
            ...
            // NOTE: Upload foto surat OPSIONAL dari form field
            // Validasi sudah ada di SubmitSikRequest (nullable)
